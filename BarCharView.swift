...
class BarChartView: UIView {
    ...
    var dataEntries: [BarEntry] = [] {
        didSet {
            ...
            mainLayer.frame = CGRect(x: 0, y: 70, width:
    ...
    private func showEntry(index: Int, entry: BarEntry) {
        let xPos: CGFloat = translateWidthValueToXPosition(value:
            Float(entry.value) / Float(100.0))
        let yPos: CGFloat = space + CGFloat(index) * (barHeight + space)
        drawBar(xPos: xPos, yPos: yPos)
        drawTextValue(xPos: xPos + 155.0, yPos: yPos + 4.0, textValue: "\(entry.value)")
        drawTitle(xPos: 16.0, yPos: yPos + 12.0, width: 150.0, height: 40.0, title: entry.title)
    }
    
    private func drawBar(xPos: CGFloat, yPos: CGFloat) {
        let barLayer = CALayer()
        barLayer.frame = CGRect(x: 144.0, y: yPos, width: xPos, height: 40.0)
        //barLayer.backgroundColor = UIColor.purple.cgColor
        barLayer.backgroundColor = UIColor(named: "inputOneColor")?.cgColor
